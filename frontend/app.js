...
function patientFolderView(patientId) {
  ...
  <div class="patient-folder-grid">
    ...
    <section class="panel patient-folder-section patient-folder-wide"><div class="panel-header"><div><h2 class="panel-title">Histórico de consentimento</h2><p class="panel-subtitle">Manifestações e versões dos termos preservadas para rastreabilidade.</p></div><span class="guide-type-tag">${consentHistory.length} evento(s)</span></div>${consentHistory.length ? `<div class="patient-simple-list consent-history">${consentHistory.map(item => `<div><strong>${item.status === 'granted' ? 'Consentimento concedido' : 'Consentimento revogado'}</strong><small>${new Date(`${item.eventDate}T12:00:00`).toLocaleDateString('pt-BR')} · registrado por ${item.recordedBy}${item.notes ? ` · ${item.notes}` : ''}</small>${item.documentHash ? `<small>Versão: ${item.consentTitle || 'Termo de consentimento'} · hash ${item.documentHash.slice(0, 12)}…</small>` : '<small>Registro anterior ao controle de versões.</small>'}<span class="status ${item.status === 'granted' ? 'approved' : 'error'}">${item.status === 'granted' ? 'Concedido' : 'Revogado'}</span>${item.documentHash ? `<button class="text-button" data-action="print-historical-consent" data-consent-id="${item.id}">Baixar versão</button>` : ''}</div>`).join('')}</div>` : empty('Nenhuma manifestação foi registrada no histórico.')}
