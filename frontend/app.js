...
async function downloadHistoricalConsentPdf(consentId) {
  if (!activeSession?.token) { showToast('Entre pela API para baixar esta versão.'); return; }
  try {
    const response = await fetch(`${apiBase}/patient-consents/${encodeURIComponent(consentId)}/pdf`, { headers: apiHeaders() });
    if (!response.ok) { const payload = await response.json().catch(() => ({})); throw new Error(payload.error || 'Não foi possível gerar a versão histórica.'); }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    const link = window.document.createElement('a');
    link.href = url; link.download = `termo-consentimento-${consentId}.pdf`; link.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  } catch (error) { showToast(error.message); }
}
let tussSearchTimer;
...
function overview() {
  ...
  const onboarding = activeUser?.role === 'admin' ? clinicOnboardingChecklist(clinicSettings, insurers, users, patients) : [];
...
function editPatient(patientId) {
  ...
  if (!form) return;
  let activeToggle = form.querySelector('#new-patient-active');
  if (!activeToggle) {
    const label = document.createElement('label');
    label.className = 'owner-active patient-active-toggle';
    label.innerHTML = '<input id="new-patient-active" name="active" type="checkbox" /> Paciente ativo para novas guias, autorizações e atendimentos';
    form.querySelector('.form-footer')?.before(label);
    activeToggle = label.querySelector('input');
  }
  ...
  activeToggle.checked = isActivePatient(patient);
...
function patientFolderView(patientId) {
  ...
  <div class="patient-folder-grid">
    ...
    <section class="panel patient-folder-section patient-folder-wide"><div class="panel-header"><div><h2 class="panel-title">Histórico de consentimento</h2><p class="panel-subtitle">Manifestações, versões e comprovantes assinados preservados para rastreabilidade.</p></div><span class="guide-type-tag">${consentHistory.length} evento(s)</span></div>${consentHistory.length ? `<div class="patient-simple-list consent-history">${consentHistory.map(item => `<div><strong>${item.status === 'granted' ? 'Consentimento concedido' : 'Consentimento revogado'}</strong><small>${new Date(`${item.eventDate}T12:00:00`).toLocaleDateString('pt-BR')} · registrado por ${item.recordedBy}${item.notes ? ` · ${item.notes}` : ''}</small>${item.documentHash ? `<small>Versão: ${item.consentTitle || 'Termo de consentimento'} · hash ${item.documentHash.slice(0, 12)}…</small>` : '<small>Registro anterior ao controle de versões.</small>'}${item.signedDocumentId ? `<small>Comprovante: ${item.signedDocumentName}</small>` : '<small>Sem comprovante assinado vinculado.</small>'}<span class="status ${item.status === 'granted' ? 'approved' : 'error'}">${item.status === 'granted' ? 'Concedido' : 'Revogado'}</span>${consentHistoryActions(item, documents)}</div>`).join('')}</div>` : empty('Nenhuma manifestação foi registrada no histórico.')}
      <form class="patient-consent-form" id="patient-consent-form" data-patient-id="${patient.id}"><div class="form-grid"><div class="field"><label for="consent-event-status">Manifestação *</label><select id="consent-event-status" name="status" required><option value="granted">Consentimento concedido</option><option value="revoked">Consentimento revogado</option></select></div><div class="field"><label for="consent-event-date">Data *</label><input id="consent-event-date" name="eventDate" type="date" value="${new Date().toISOString().slice(0, 10)}" required /></div><div class="field"><label for="consent-signed-document">Comprovante assinado</label><select id="consent-signed-document" name="signedDocumentId"><option value="">Registrar sem comprovante</option>${documents.map(item => `<option value="${item.id}">${item.originalName} · ${item.category}</option>`).join('')}</select><small>Anexe o termo assinado na seção de documentos antes de registrar.</small></div><div class="field consent-notes-field"><label for="consent-event-notes">Observação</label><input id="consent-event-notes" name="notes" maxlength="500" placeholder="Ex.: termo conferido pela recepção" /></div></div><div class="form-footer"><button class="primary-button" type="submit" ${activeSession?.token ? '' : 'disabled'}>Registrar manifestação</button></div></form>
    </section>
...
function reportsView() {
  ...
  const production = guideProductionSummary(reportGuides);
  const productionRows = items => items.length ? items.slice(0, 8).map((item, index) => `<div class="production-ranking-row"><span class="production-ranking-position">${index + 1}</span><div><strong>${escapeHtml(item.label)}</strong><small>${item.guides} guia(s) · ${formatMoney(item.valueCents / 100)}</small></div><b>${item.sessions} atendimento(s)</b></div>`).join('') : '<p class="panel-subtitle">Nenhuma produção encontrada na competência.</p>';

  return `<div class="page-heading"><div><p class="eyebrow">Indicadores operacionais</p><h1>Relatórios</h1><p class="heading-copy">Acompanhe faturamento, produção assistencial e conformidade em uma única competência.</p></div><label class="report-competence">Competência <input id="report-competence" type="month" value="${selectedReportCompetence}" /></label></div><div class="report-export-bar"><span>Exportar CSV compatível com Excel</span><button class="secondary-button" data-report-export="production">Produção detalhada</button><button class="secondary-button" data-report-export="guides">Guias</button><button class="secondary-button" data-report-export="batches">Lotes e recebimentos</button><button class="secondary-button" data-report-export="invoices">Notas fiscais</button><button class="secondary-button" data-report-export="glosas">Glosas</button><button class="secondary-button" data-report-export="authorizations">Autorizações</button>${activeUser?.role === 'admin' ? '<button class="secondary-button" data-report-export="consents">Consentimentos</button>' : ''}</div>
  ...
  <div class="stats-grid"><article class="stat-card"><div class="stat-top"><span>Atendimentos faturados</span><span class="stat-icon">◎</span></div><div class="stat-value">${production.sessions}</div><div class="stat-note">Em ${production.guides} guia(s)</div></article><article class="stat-card"><div class="stat-top"><span>Guias aprovadas</span><span class="stat-icon">◉</span></div><div class="stat-value">${guideCounts.approved || 0}</div><div class="stat-note">Processadas com sucesso</div></article><article class="stat-card"><div class="stat-top"><span>Valor pendente</span><span class="stat-icon">◷</span></div><div class="stat-value">${formatMoney(pendingAmount)}</div><div class="stat-note warn">${formatMoney(receivedAmount)} recebido(s)</div></article><article class="stat-card"><div class="stat-top"><span>Valor em glosa</span><span class="stat-icon">✕</span></div><div class="stat-value">${formatMoney(openGlosaAmount)}</div><div class="stat-note ${openGlosas.length ? 'warn' : ''}">${openGlosas.length} glosa(s) em aberto ou recurso</div></article></div>
  <div class="content-grid production-grid"><div class="panel"><div class="panel-header"><div><h2 class="panel-title">Produção por profissional</h2><p class="panel-subtitle">Atendimentos presentes nas guias da competência.</p></div></div><div class="production-ranking">${productionRows(production.byProfessional)}</div></div><div class="panel"><div class="panel-header"><div><h2 class="panel-title">Produção por paciente</h2><p class="panel-subtitle">Volume faturado sem expor dados fora da clínica.</p></div></div><div class="production-ranking">${productionRows(production.byPatient)}</div></div></div>
  ...
  return `<div class="page-heading"><div><p class="eyebrow">Identidade dos documentos</p><h1>Configurações da clínica</h1><p class="heading-copy">Estes dados serão usados na capa e na guia impressa.</p></div><button class="secondary-button" data-action="download-backup">Baixar backup da clínica</button></div>
    ...
    <form class="panel patient-form" id="settings-form">
      ...
      <div class="form-section">
        ...
        <div class="consent-settings"><div class="owners-settings-heading"><div><label>Termo de consentimento</label><small>Use somente um texto revisado para as práticas da clínica. Deixe vazio para manter o modelo administrativo padrão.</small></div></div><div class="form-grid"><div class="field"><label>Título do documento</label><input name="consentTitle" maxlength="120" value="${clinicSettings.consentTitle || ''}" placeholder="Termo de ciência e consentimento" /></div><div class="field"><label>Contato de privacidade</label><input name="privacyContact" maxlength="180" value="${clinicSettings.privacyContact || ''}" placeholder="E-mail ou telefone para solicitações" /></div><div class="field"><label>Renovar o consentimento após</label><select name="consentRenewalMonths"><option value="0" ${!clinicSettings.consentRenewalMonths ? 'selected' : ''}>Não controlar vencimento</option><option value="6" ${Number(clinicSettings.consentRenewalMonths) === 6 ? 'selected' : ''}>6 meses</option><option value="12" ${Number(clinicSettings.consentRenewalMonths) === 12 ? 'selected' : ''}>12 meses</option><option value="24" ${Number(clinicSettings.consentRenewalMonths) === 24 ? 'selected' : ''}>24 meses</option><option value="36" ${Number(clinicSettings.consentRenewalMonths) === 36 ? 'selected' : ''}>36 meses</option></select><small>Opcional. O sistema avisa 30 dias antes do prazo escolhido.</small></div><div class="field consent-text-field"><label>Texto revisado do termo</label><textarea name="consentText" maxlength="5000" rows="9" placeholder="Escreva o texto aprovado pela clínica. Separe os parágrafos com uma linha em branco.">${clinicSettings.consentText || ''}</textarea><small>O sistema acrescenta automaticamente identificação, situação registrada e campos de assinatura.</small></div></div></div>
...
document.addEventListener('submit', async event => {
  ...
  if (event.target.dataset.patientId) {
    ...
    event.stopImmediatePropagation();
    const data = Object.fromEntries(new FormData(event.target));
    data.active = event.target.querySelector('#new-patient-active')?.checked ?? true;
    try {
      if (activeSession?.token) {
...
document.addEventListener('click', async event => {
  ...
  const historicalConsentButton = event.target.closest('[data-action="print-historical-consent"]');
  if (historicalConsentButton) {
    await downloadHistoricalConsentPdf(historicalConsentButton.dataset.consentId);
    return;
  }
...
document.addEventListener('submit', async event => {
  if (!event.target.classList.contains('consent-document-form')) return;
  event.preventDefault();
  const form = event.target;
  const patientId = patientConsents.find(item => item.id === form.dataset.consentId)?.patientId;
  const documentId = new FormData(form).get('documentId') || '';
  const submitButton = form.querySelector('button[type="submit"]');
  submitButton.disabled = true;
  try {
    await apiRequest(`/patient-consents/${encodeURIComponent(form.dataset.consentId)}/document`, { method: 'PATCH', body: JSON.stringify({ documentId }) });
    patientConsents = await apiRequest('/patient-consents');
    if (patientId) appView.innerHTML = patientFolderView(patientId);
    showToast(documentId ? 'Comprovante vinculado ao consentimento.' : 'Comprovante desvinculado do consentimento.');
  } catch (error) {
...
async function downloadCsvReport(type) {
  ...
  const query = type !== 'consents' && selectedReportCompetence ? `?competence=${encodeURIComponent(selectedReportCompetence)}` : '';
  try {
