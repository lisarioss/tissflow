...
const defaultGuides = [
  { id: 'G-2026-00481', patient: 'Helena Martins', procedure: 'Consulta ambulatorial', insurer: 'Unimed', status: 'approved', label: 'Aprovada', date: dataRelativaCurta(-2), value: 'R$ 180,00' },
  { id: 'G-2026-00480', patient: 'Rafael Nogueira', procedure: 'Sessão de fisioterapia', insurer: 'Bradesco Saúde', status: 'review', label: 'Em análise', date: dataRelativaCurta(-2), value: 'R$ 120,00' },
  { id: 'G-2026-00479', patient: 'Bianca Torres', procedure: 'Ultrassonografia', insurer: 'SulAmérica', status: 'error', label: 'Com pendência', date: dataRelativaCurta(-3), value: 'R$ 260,00' },
  { id: 'G-2026-00478', patient: 'João Pedro Lima', procedure: 'Consulta ambulatorial', insurer: 'Amil', status: 'sent', label: 'Enviada', date: dataRelativaCurta(-3), value: 'R$ 180,00' }
];
let guides = JSON.parse(localStorage.getItem(clinicStorageKey('guides')) || 'null') || defaultGuides;
const defaultPatients = [
  { id: 'P-001', name: 'Helena Martins', birthDate: '1988-03-14', insurer: 'Unimed', ansCode: '004701', cardNumber: '0123456789012', plan: 'Unimed Nacional Apartamento', planValidity: `${anoSeguinte()}-12-31` },
  { id: 'P-002', name: 'Rafael Nogueira', birthDate: '2014-09-22', insurer: 'Bradesco Saúde', ansCode: '005711', cardNumber: '9876543210001', plan: 'Bradesco Efetivo', planValidity: `${anoSeguinte()}-06-30` },
  { id: 'P-003', name: 'Bianca Torres', birthDate: '1992-11-08', insurer: 'SulAmérica', ansCode: '006246', cardNumber: '2468135790004', plan: 'SulAmérica Exato', planValidity: `${anoAtual()}-12-31` }
];
let patients = JSON.parse(localStorage.getItem(clinicStorageKey('patients')) || 'null') || defaultPatients;
const defaultInvoices = [
  { id: 'NF-2026-001', guideId: 'G-2026-00481', provider: 'Apex Equipamentos', description: 'Material de consumo', amount: 1280, expectedDate: dataRelativaIso(0), status: 'pending' },
  { id: 'NF-2026-002', guideId: 'G-2026-00478', provider: 'MediSupply', description: 'Produtos de apoio clínico', amount: 980, expectedDate: dataRelativaIso(2), status: 'received' },
  { id: 'NF-2026-003', guideId: '', provider: 'Lab Plus', description: 'Reagentes e exames', amount: 2400, expectedDate: dataRelativaIso(4), status: 'pending' }
];
let invoices = JSON.parse(localStorage.getItem(clinicStorageKey('invoices')) || 'null') || defaultInvoices;
const defaultAppointments = [
  { id: 'A-001', patient: 'Helena Martins', professional: 'Marina Souza', date: hojeIso(), start: '09:00', duration: 50, type: 'Consulta' },
  { id: 'A-002', patient: 'Rafael Nogueira', professional: 'Marina Souza', date: hojeIso(), start: '10:00', duration: 50, type: 'Consulta' },
  { id: 'A-003', patient: 'Bianca Torres', professional: 'Lucas Andrade', date: hojeIso(), start: '09:30', duration: 60, type: 'Fisioterapia' }
];
...
function overview() {
  return `<div class="page-heading"><div><p class="eyebrow">${capitalizar(diaDaSemanaLongo(hojeIso()))}</p><h1>${saudacaoInstantanea(activeUser?.name)}</h1><p class="heading-copy">Aqui está o pulso do faturamento da sua clínica.</p></div><button class="primary-button" data-action="new-guide">＋ Nova guia</button></div>
  ...
  <div class="panel" style="margin-top:18px"><div class="panel-header"><div><h2 class="panel-title">Volume de guias</h2><p class="panel-subtitle">Guias processadas nos últimos 7 dias</p></div><button class="secondary-button">Últimos 7 dias⌄</button></div><div class="chart-area"><div class="chart-grid"><svg class="chart-line" viewBox="0 0 700 140" preserveAspectRatio="none"><polyline points="0,112 100,91 200,101 300,65 400,78 500,34 600,49 700,15"/><circle cx="0" cy="112" r="3"/><circle cx="100" cy="91" r="3"/><circle cx="200" cy="101" r="3"/><circle cx="300" cy="65" r="3"/><circle cx="400" cy="78" r="3"/><circle cx="500" cy="34" r="3"/><circle cx="600" cy="49" r="3"/><circle cx="700" cy="15" r="3"/></svg></div><div class="chart-labels">${ultimosSeteDias().map(rotulo => `<span>${rotulo}</span>`).join('')}</div></div></div>`;
}
function guideFormComplete() {
  ...
  <form class="guide-form" id="guide-form">
    <div class="guide-plan-header"><div class="plan-logo"><img id="plan-logo-image" src="" alt="Logo da operadora" hidden /><span id="plan-logo">TISS</span></div><div><strong id="plan-name">Selecione a operadora</strong><small>Guia SP/SADT · padrão demonstrativo</small></div><span class="guide-code">Nº <b>G-${anoAtual()}-DEMO</b></span></div>
...
<mensagemTISS versao="4.01.00">
  <cabecalho><identificacaoTransacao><tipoTransacao>ENVIO_LOTE_GUIAS</tipoTransacao><sequencialTransacao>000001</sequencialTransacao><dataRegistroTransacao>${escapeXml(data.date || hojeIso())}</dataRegistroTransacao><horaRegistroTransacao>10:30:00</horaRegistroTransacao></identificacaoTransacao></cabecalho>
...
document.addEventListener('submit', async event => {
  ...
  const guideId = nextSequentialId(guides, `G-${anoAtual()}-`, 5);
  try {
