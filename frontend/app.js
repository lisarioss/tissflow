...
function guideList() { return `<div class="page-heading"><div><p class="eyebrow">Operação de faturamento</p><h1>Guias TISS</h1><p class="heading-copy">Acompanhe o ciclo de cada guia, do preenchimento ao envio.</p></div><div class="folder-actions"><button class="secondary-button" data-action="new-guide" data-guide-type="consulta">＋ Nova guia de Consulta</button><button class="primary-button" data-action="new-guide" data-guide-type="sp_sadt">＋ Nova guia SP/SADT</button></div></div><div class="panel"><div class="panel-header"><div><h2 class="panel-title">Todas as guias</h2><p class="panel-subtitle">${guides.length} registros salvos neste navegador</p></div><button class="secondary-button" data-action="clear-guides">Limpar dados demo</button></div><table><thead><tr><th>Guia</th><th>Paciente</th><th>Convênio</th><th>Tipo</th><th>Status</th><th>Valor</th><th></th></tr></thead><tbody>${guides.map(g => `<tr><td><strong>${g.id}</strong><small>${g.date}</small></td><td>${g.patient}<small>${g.procedure}</small></td><td>${g.insurer}</td><td><span class="guide-type-tag">${g.guideType === 'consulta' ? 'Consulta' : 'SP/SADT'}</span></td><td>${statusTag(g)}</td><td><strong>${g.value}</strong></td><td><button class="text-button" data-action="open-guide-folder" data-guide-id="${g.id}">Abrir pasta →</button></td></tr>`).join('')}</tbody></table></div>${statusSimulator()}`; }
